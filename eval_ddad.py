...
pretrained_dict = torch.load("pretrained_model/DDAD/model_DDAD.pth")
...
with torch.no_grad():
    for batch_idx, inputs in enumerate(data_loader):
        ...
        outputs = model(imgs[0], imgs[1:], pose_mats[0], pose_mats[1:],
                             inputs[('inv_K_pool', 0)])
        depth_pred_1 = outputs[('depth_pred', 0)]
        depth_pred_2 = outputs[('depth_pred_2', 0)]

        depth_pred_2_np = depth_pred_2.cpu().detach().numpy().squeeze()
        depth_pred_1_np = depth_pred_1.cpu().detach().numpy().squeeze()

        error_temp = compute_errors_perimage(depth_gt_np[mask], depth_pred_1_np[mask], min_depth, max_depth)
        error_temp_2_ = compute_errors_perimage(depth_gt_np[mask], depth_pred_2_np[mask], min_depth, max_depth)
